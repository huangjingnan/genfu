...
jQuery.fn.ingrid = function(o){
	...
	// create header table
	var cols = new Array();
	var h = jQuery('<table cellpadding="0" cellspacing="0" width="100%"></table>')
		...
		initStylesAndWidths : function() {
			...
			this.getRows().each(function(r){
				...
				jQuery(this).find('td').each(function(i){
					...
					// add genfu
					jQuery(this).find("a[name='view_edit']").each(function(j){
						jQuery(this).click(function() {
							//alert('click a');
							//alert(jQuery(this).attr("href").substring(1));
							viewModelGenfu(jQuery(this).attr("href").substring(1));
						});
					});
					// add genfu
					jQuery(this).find("a[name='delete']").each(function(j){
						jQuery(this).click(function() {
							//var params 	= {};
							//params.page = 3;
							//alert('click delete');
							...
							pload.removeClass(cfg.pageLoadingDoneClass);
							for (i=0; i<rows.length; i++) {
								//alert(jQuery(rows[i]).attr('id'));
								//alert(jQuery(rows[i]).html());
								//alert(jQuery(rows[i]).find("td a[name='delete']").attr('href'));
								deleteModelGenfu(jQuery(rows[i]).find("td a[name='delete']").attr('href').substring(1));
								//if ( jQuery(rows[i]).attr('id') ) row_ids.push( jQuery(rows[i]).attr('id') );
					...
					// add genfu
					jQuery(this).find("a[name='delete_new']").each(function(j){
						jQuery(this).click(function() {
							//var params 	= {};
							//params.page = 3;
							//alert('click delete');
							...
							if(rows.length>0){
								...
								for (i=0; i<rows.length; i++) {
									//alert(jQuery(rows[i]).attr('id'));
									//alert(jQuery(rows[i]).html());
									//alert(jQuery(rows[i]).find("td [title='actor_id']").html());
								...
								$.ajax({
									...
									data: {
										className: jQuery(rows[0]).find("td a[name='delete_new']").attr('href').substring(1)
									},
									type : "POST",
									beforeSend : function() {
										//$("#large").html('');
								...
								//deleteModelsGenfu(ids,jQuery(rows[0]).find("td a[name='delete_new']").attr('href').substring(1));
								//pload.addClass(cfg.pageLoadingDoneClass);
								//sleep(2000);
								//g.load();
							}
					...
					// add genfu
					jQuery(this).find('input').each(function(k){
						jQuery(this).click(function() {
