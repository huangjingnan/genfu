...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		addLocalData = function() {
			...
			svgAllQueryResults = queryResults,
		...
		return this.each(function(){
			...
			function postItWithFile() {
				...
				if(ret[0] && !rp_ge[$t.p.id].processing) {
					...
					if(rp_ge[$t.p.id].url.indexOf('?_method=put')>0){
						//debugger;
						rp_ge[$t.p.id].url = rp_ge[$t.p.id].url.replace(/[0-9]+\?/,postdata[idname]+'?');
					}
						...
						complete:function(data,status){
							...
							if(data.status >= 300 && data.status !== 304) {
								...
								if(ret === undefined) {
									ret = [true,"",""];
									//genfu add
									try{ret[2] = JSON.parse(data.responseText).id;} catch (e){}
									//ret[2] = JSON.parse(data.responseText).id;
								}
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				$($t.p.colModel).each( function() {
					if(this.edittype !== undefined && this.edittype === "file"){
						genfuFile = true;
						return false;
					}
				});
				if(genfuFile === true){
					var dh = isNaN(p.dataheight) ? p.dataheight : p.dataheight+"px",
							...
							frm = $("<form name='FormPost' id='"+frmgr+"' class='FormGrid' accept-charset='UTF-8' enctype='multipart/form-data' style='width:"+dw+";overflow:auto;position:relative;height:"+dh+";'></form>").data("disabled",false),
	...
	delGridRow : function(rowids,p) {
		...
		return this.each(function(){
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				p.gbox = "#gbox_"+$.jgrid.jqID(gID);
				//genfu change order createModal after onBeforeInit
				if(onBeforeInit) {
					showFrm = onBeforeInit.call($t,$("#"+dtbl));
					if(showFrm === undefined) {
						showFrm = true;
