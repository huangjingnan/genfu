...
<form method="post" id="authorization" action="%{#ctx}/authorization">
	<fieldset>
...
<script type="text/javascript">
	...
	function validValue(){
		...
		if (null === document.all.order_id.getAttribute('disabled')) {
			...
			myurl = '${ctx}/authorization/' + $("#order_id").val() + '?_method=put';

		} else if ("disabled" === document.all.order_id
				.getAttribute('disabled')) {
			myurl = '${ctx}/authorization';
		} else {
			valid = false;
		}
	...
	function render2() {
		validValue();
		$.ajax({
			url : '${ctx}/authorization.json?status_EQ=OPEN',
			async : false,
			dataType : 'json',
			data : {
