...
public class CatalogController extends ValidationAwareSupport implements
	...
	public HttpHeaders index() {
		...
		myparameters.put("FIRST_RESULT", (limit * (page - 1)) < 0 ? 0 : limit
				* page - limit);
		int records = 0;

		if (parameters.containsKey("tagId") && null != parameters.get("tagId")
			...
			list = genfuCommonService.searchList(
					"from Dish WHERE dishFlag=:dishFlag0 ORDER BY id ASC",
					myparameters, Dish.class);
		}
		...
		if (records > 1) {
			...
			StringBuffer pager = new StringBuffer("<div class=\"pagination\">");
			if (page > 1) {
