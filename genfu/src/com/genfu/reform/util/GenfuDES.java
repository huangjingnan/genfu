...
public class GenfuDES {
	...
	// 构造函数，简单实现
	public GenfuDES(String strDesKey) {
		if (null != strDesKey) {
			byte[] byteDesKey = strDesKey.getBytes();
			if (byteDesKey.length > 56) {

			}
			this.desKey = new SecretKeySpec(byteDesKey, "DES");
		} else {
	...
	public String encrypt2(String text) {
		...
		try {
			aliceCipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
			aliceCipher.init(Cipher.ENCRYPT_MODE, desKey);
			byte[] cleartext = text.getBytes();
			byte[] ciphertext = aliceCipher.doFinal(cleartext);
	...
	public boolean ifCorrect(String password) {
	...
	public boolean ifCorrect(byte[] ciphertext, String passwd) throws Exception {

		Cipher aliceCipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
		aliceCipher.init(Cipher.DECRYPT_MODE, desKey);
		byte[] recovered = aliceCipher.doFinal(ciphertext);

		if (!java.util.Arrays.equals(passwd.getBytes(), recovered)) {
	...
	private void run() throws Exception {
		...
		try {
			...
			// DHParameterSpec dhSkipParamSpec = new DHParameterSpec(
			...
			//
			// aliceKeyAgree.doPhase(alicePubKey, true);
			// SecretKey aliceDesKey = aliceKeyAgree.generateSecret("DES");
			SecretKey genfuDesKey = new SecretKeySpec("12345678".getBytes(),
					"DES");
			// DESKeySpec genfuDESKeyS = new DESKeySpec("12345678".getBytes());

			Cipher aliceCipher = Cipher.getInstance("DES/ECB/PKCS5Padding");
			aliceCipher.init(Cipher.ENCRYPT_MODE, genfuDesKey);

			byte[] cleartext = "This is just an example".getBytes();
			byte[] ciphertext = aliceCipher.doFinal(cleartext);

			aliceCipher.init(Cipher.DECRYPT_MODE, genfuDesKey);
			byte[] recovered = aliceCipher.doFinal(ciphertext);
			...
			System.out.println("DES in ECB mode recovered text is "
					+ "same as cleartext");
			System.out.println("Alice secret: " + toHexString(recovered));
