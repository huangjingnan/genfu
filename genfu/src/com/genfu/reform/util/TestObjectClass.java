...
public class TestObjectClass {
	...
	public static void main(String[] args) {
		...
		// testReflect();
		try {
			Class<?> theClass = Class.forName("com.genfu.reform.model.Dish");
			// Object theObject = theClass.newInstance();
			// theObject.getClass().getFields();
			Field[] fds = theClass.getDeclaredFields();
			Field fd = theClass.getDeclaredField("price");
			Object val = "0";

			// fd.getType().cast(val);

			val = new java.math.BigDecimal((String) val);
			if ("long".equals(fd.getType().getName())) {
				System.out.println();
			}
			for (int i = 0; i < fds.length; i++) {
				System.out.print(fds[i].getName());
				System.out.print(fds[i].getType().getName());
